...
void CUser::AddPartyList( vector<PARTYFINDER_LIST> vectmp, int nCount )
{
	...
	{
		m_Snapshot.ar << vectmp[i].m_uPartyId;								// 극단 ID
		m_Snapshot.ar.WriteString( vectmp[i].m_sParty );							// 극단 명칭( 단막극단 : NO, 순회극단 : YES )
		m_Snapshot.ar << vectmp[i].m_nSizeofMember;						// 극단원 숫자	( 2 ~ 8 )
		m_Snapshot.ar << vectmp[i].m_nLevel << vectmp[i].m_nPoint;				// 극단 레벨, 경험치, 포인트
