	...
	for( C2PartyPtr::iterator i = g_PartyMng.m_2PartyPtr.begin(); i != g_PartyMng.m_2PartyPtr.end(); i++ )
	{
		...
		if( pParty->GetLeader() )
		 ...
		 if( pParty->GetSizeofMember() < MAX_PTMEMBER_SIZE_SPECIAL )
		 {
			...
			partytmp.m_uPartyId = pParty->m_uPartyId;								// 극단 ID
			strcpy( partytmp.m_sParty, pParty->m_sParty );							// 극단 명칭( 단막극단 : NO, 순회극단 : YES )
			partytmp.m_nSizeofMember = pParty->m_nSizeofMember	;								// 극단원 숫자	( 2 ~ 8 )
