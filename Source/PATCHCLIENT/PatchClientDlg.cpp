...
CPatchClientDlg *g_pDlg;

/////////////////////////////////////////////////////////////////////////////
// CPatchClientDlg dialog
...
/////////////////////////////////////////////////////////////////////////////
// CPatchClientDlg message handlers
...
void CPatchClientDlg::OnDestroy() 
{
	if( m_pHttpDownload )
	{
...
UINT CPatchClientDlg::OnNcHitTest( CPoint point )
{
	// 캡션바를 누른것 처럼 해주는 것이다.
	return HTCAPTION;
}

LRESULT CPatchClientDlg::OnHttpDownloadThreadFinished( WPARAM wParam, LPARAM lParam )
{
...
LRESULT CPatchClientDlg::OnHttpDownloadFail( WPARAM wParam, LPARAM lParam )
{
	m_pHttpDownload->WaitForSafeToClose();
	PostQuitMessage( 0 );
	return 0;
...
void CPatchClientDlg::OnButtonStart() 
{
#if __CURRENT_LANG == LANG_KOR
#ifdef __LINK_PORTAL
	//무인자 일 경우 홈페이지로 연결
...
//		AfxMessageBox( strCmdLine );
	...
	{
		CPatchClientApp* pApp = (CPatchClientApp*)AfxGetApp();
		pApp->RunClient();
	}
}

void CPatchClientDlg::OnButtonRegister()
{
#if defined(__LANG_ENG_USA) || defined(__LANG_ENG_USATEST)
	::ShellExecute(NULL, "open", "http://www.YOUR-WEBSITE.com/?page=register", NULL, ".", SW_SHOWNORMAL); //rev1
...
BOOL CPatchClientDlg::InitControls()
{
#ifdef __LANG_RUS
...
void CPatchClientDlg::NewPatchClient()
{
	STARTUPINFO si;
...
void CPatchClientDlg::SetPatchVersionString( int nVersion )
{
	char szBuffer[256];
	sprintf( szBuffer, "FlyFF %.1f %s", nVersion/10.0f, VERSION_NAME ); //rev1
	m_Static_MasqueradeVersion.SetText( szBuffer );
}

void CPatchClientDlg::CreateWebControl( LPCTSTR szURL )
{
