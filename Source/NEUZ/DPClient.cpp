...
void CDPClient::OnRefreshPartyList( CAr & ar )
{
	...
	for( int i=0; i<nCount; i++ )
	{
		...
		ar >> tmplist.m_uPartyId;								// 극단 ID
		ar.ReadString( tmplist.m_sParty );							// 극단 명칭( 단막극단 : NO, 순회극단 : YES )
		ar >> tmplist.m_nSizeofMember;						// 극단원 숫자	( 2 ~ 8 )
		ar >> tmplist.m_nLevel >> tmplist.m_nPoint;				// 극단 레벨, 경험치, 포인트
